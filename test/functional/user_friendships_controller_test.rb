...
class UserFriendshipsControllerTest < ActionController::TestCase
	...
	context "#index" do
		...
		context "when logged in" do
			setup do
				@friendship1 = create(:pending_user_friendship, user: users(:karina), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
				@friendship2 = create(:accepted_user_friendship, user: users(:karina), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
				@friendship3 = create(:requested_user_friendship, user: users(:karina), friend: create(:user, first_name: 'Requested', last_name: 'Friend'))
				@friendship4 = user_friendships(:blocked_by_karina)
	...
	context "#new" do
		...
		context "when logged in" do	
			...
			should "display the friend's name" do
				...
				assert_match /#{users(:bob).full_name}/, reponse.body
			end
			...
			should "assign a new user friendship to the currently logged in user" do
				get :new, friend_id: users(:bob)
				assert_equal users(:bob), assigns(:user_friendship).user
			end
			...
			context "successfully" do
				should "create two user friendship objects" do
					assert_difference 'UserFriendship.count', 2 do
						post :create, user_friendship: { friend_id: users(:bob).profile_name }
					end
				end
			end
			context "with a valid friend_id" do
				setup do
					post :create, user_friendship: { friend_id: users(:bob) }
				end
				should "assign a friend object" do
					assert assigns(:friend)
					assert_equal users(:bob), assigns(:friend)
				end
				should "assign a user_friendship object" do
					assert assigns(:user_friendship)
					assert_equal users(:karina), assigns(:user_friendship).user
					assert_equal users(:bob), assigns(:user_friendship).friend
				end
				should "create a friendship" do
					assert users(:karina).pending_friends.include?(users(:bob))
				end
				should "redirect to the profile page of the friend" do
					assert_response :redirect
					assert_redirected_to profile_path(users(:bob))
				end
	...
	context "#accept" do
		...
		context "when logged in" do
			setup do
				@friend = create(:user)
				@user_friendship = create(:pending_user_friendship, user: users(:karina), friend: @friend)
				create(:pending_user_friendship, friend: users(:karina), user: @friend)
				sign_in users(:karina)
			...
			should "have a flash success message" do
				assert_equal 'You are now friends with #{@user.friendship.friend.first_name}', flash[:success]
			end
		end
	end
	...
	context "#edit" do
		...
		context "when logged in" do	
			setup do
				@user_friendship = create(:pending_user_friendship, user: users(:karina))
				sign_in users(:karina)
	...
	context "#destroy" do
		...
		context "when logged in" do
			setup do
				@friend = create(:user)
				@user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:karina))
				create(:accepted_user_friendship, friend: users(:karina), user: @friend)
				sign_in users(:karina)
			end
